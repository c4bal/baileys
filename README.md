...
![Logo](https://files.catbox.moe/c5tam9.jpg)
...
- Added `@napi-rs/image` as a supported image processing backend in `getImageProcessingLibrary()`, offering a balance between performance and compatibility.
...
# NPM
"dependencies": {
   "@c4bal/baileys": "latest"
}

# GitHub
"dependencies": {
   "@c4bal/baileys": "github:c4bal/baileys"
}
```
...
```javascript
// --- ESM
import { makeWASocket } from '@c4bal/baileys'

// --- CJS (tested and working on Node.js 24)
const { makeWASocket } = require('@c4bal/baileys')
```
...
```javascript
import { makeWASocket, delay, DisconnectReason, useMultiFileAuthState } from '@c4bal/baileys'
...
```javascript
import { makeWASocket, makeInMemoryStore, delay, DisconnectReason, useMultiFileAuthState } from '@c4bal/baileys'
import { Boom } from '@hapi/boom'
import pino from 'pino'
...
sock.sendMessage(jid, {
   ...
   couponCode: '@c4bal/baileys', // --- Optional
   nativeFlow: [{
      text: '👋🏻 Greeting',
      id: '#Greeting'
   }, {
...
```javascript
// --- Create a new one
sock.newsletterCreate('@c4bal/baileys')
...
// --- Change name
sock.newsletterUpdateName('1231111111111@newsletter', '📦 @c4bal/baileys')
...
sock.groupCreate('@c4bal/baileys', ['628123456789@s.whatsapp.net'])
...
// --- Change name
sock.groupUpdateSubject(jid, '📦 @c4bal/baileys')
